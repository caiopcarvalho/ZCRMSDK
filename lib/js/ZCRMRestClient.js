...
var redirect_url = process.env.ZOHO_REDIRECT_URL;
var user_identifier = process.env.ZOHO_USER_IDENTIFIER;
var mysql_module = process.env.ZOHO_MYSQL_MODULE;
var iamurl = process.env.ZOHO_IAMURL;
var baseURL = process.env.ZOHO_BASEURL;
var version = process.env.ZOHO_VERSION;
var mysql_host = process.env.ZOHO_MYSQL_HOST;
var mysql_username = process.env.ZOHO_MYSQL_USERNAME;
var mysql_password = process.env.ZOHO_MYSQL_PASSWORD;
var default_user_identifier = process.env.ZOHO_DEFAULT_USER_IDENTIFIER;
        ...
        OAuth.generateTokens(api_url)
            .then(function(response) {
                ...
                if (resultObj.access_token) {
                    mysql_util.saveOAuthTokens(resultObj)
                        .then(function(save_resp) {
...
ZCRMRestClient.getOAuthTokens = function() {
    return new Promise((resolve, reject) => {
        var mysql_util = require(mysql_module);
        mysql_util.getOAuthTokens()
            .then(function(mysql_response) {
                resolve(mysql_response[0].accesstoken);
            });
    });
}
