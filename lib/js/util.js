function promiseResponse(request)
{
  ...
  return new Promise(function(resolve,reject)
  {
    ...
    //console.log(crmclient.getUserIdentifier());
    mysql_util.getOAuthTokens(crmclient.getUserIdentifier())
    .then(function(response)
    {
      ...
      // console.log({ current_time, expires_in, c: current_time > expires_in, diff: current_time - expires_in })
      ...
      if(current_time > expires_in)
      {
        ...
        OAuth.generateTokens(url)
        .then(function(response)
        {
          var result_obj = crmclient.parseAndConstructObject(response);
          if (result_obj.hasOwnProperty("access_token") && result_obj.hasOwnProperty("expires_in")) {
            mysql_util.updateOAuthTokens(result_obj)
            .then(function(response)
            {
              makeapicall(request)
              .then(function(response)
...
function makeapicall(request)
{
  return new Promise(function(resolve,reject)
  {
    ...
    .then(function(result_obj)
    {
      ...
      // Does api URL contain bigin, or crm?
      let baseProduct = crmclient.getAPIURL().indexOf('bigin') >= 0 || crmclient.getAPIURL().indexOf('crm') >= 0 ? '' : 'crm'
      var baseUrl = `${crmclient.getAPIURL()}/${baseProduct}/${crmclient.getVersion()}/${request.url}`
      ...
      function(error,response,body)
      {
        ...
        if(response.statusCode == 204)
        {
          ...
          resolve({statusCode: 204, body: JSON.stringify(respObj)});
        }
        else
        {
          if (request.download_file)
